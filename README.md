...
- **Конфигурация**:
  ...
  - `DJANGO_ALLOWED_HOST` ограничены только локальными адресами (`127.0.0.1`, `localhost`).
...
- **Конфигурация**:
  ...
  - `DJANGO_ALLOWED_HOST` включает все публичные и частные адреса, где приложение будет доступно.
...
DEBUG=False
DJANGO_ALLOWED_HOSTS=127.0.0.1,localhost,your_host,xxx.xxx.xxx.xxx
```
